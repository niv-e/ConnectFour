...
namespace ConnectFourGame.API.Controllers
{
    ...
    public class ProfileManagementController : ControllerBase
    {
        ...
        public async Task<ActionResult<bool>> DeletePlayerById(int playerid)
        {
            try
            {
                return await _playerService.DeletePlayerById(playerid);
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
        ...
        public async Task<ActionResult<bool>> OnPostUpdatePlayer([FromBody]PlayerBoundary playerBoundary)
        {
            try
            {
                return await _playerService.UpdatePlayer(playerBoundary);
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
